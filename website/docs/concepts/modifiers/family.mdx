...
In this case however you can also achieve the same effect by using a single [StateNotifier] with a method that is called every time
the filter is called, allowing you to debounce the filter if it changes frequently. See the [Asynchrony Cookbook](/docs/cookbooks/asynchrony) for an example of this.
...
Here's two examples using [Freezed] and [equatable]:
